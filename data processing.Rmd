...
w2 <- read_sav("Data/MASTER w2 adult and youth merged.sav")
...
## Examine Raw Inputs (check for werid and missing things)
```{r}
# Baseline
tbl_summary(
  baseline,
...
# Check
baseline_scales %>% 
  select(272:295, SCI2_score_baseline, SCI.II.Total) 
```
```{r}
# Check
w2_scales %>% 
  select(150:173, SCI2_score_w2, SCI.II.Total) 
```
```{r}
# Check
w3_scales %>% 
  select(188:211, SCI2_score_w3, SCI.II.Total) 
```
...
baseline_subset <- baseline_scales %>% 
  select(ID, Cohort, SCI2_score_baseline, Group)
...
# Fit linear model
```{r}
SCI2_did <- lm(SCI2_score ~ 
     Group + time + (Group * time), 
   data = SCI2_complete_pivot)
